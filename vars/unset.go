...
func (v *SVars) Unset(variable string) error {
	...
	resp, statusCode, err := httpclient.Delete(nil, fmt.Sprintf("%s%s/services/%s/env/%s", v.Settings.PaasHost, v.Settings.PaasHostVersion, v.Settings.ServiceID, variable), headers)
	if err != nil {
		return err
	}
